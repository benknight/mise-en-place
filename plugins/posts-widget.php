...
function CategoryPosts() {
	parent::WP_Widget(false, $name='Posts', array( 'description' => 'Show a collection of posts.' ));
}
...
function widget($args, $instance) {
  ...
  if ( in_array($instance['sort_by'], $valid_sort_orders) ) {
	...
	// output nothing if we don't have posts.
	if ( ! $cat_posts->have_posts() )
		return;

	// Excerpt length filter
	if ( (int) $instance["excerpt_length"] > 0 ) {
		$new_excerpt_length = create_function('$length', "return (int) " . $instance["excerpt_length"] . ";");
	...
	if( $instance["title_link"] ) {
		$category_link = $instance['cat'] ? get_category_link($instance["cat"]) : get_page_link( get_option( 'page_for_posts' ) );
		echo '<a href="' . $category_link . '">' . $instance["title"] . '</a>';
	} else {
		echo $instance["title"];
	}
	echo $after_title;
	
	echo '<div class="cat-post-items">';
	...
	while ( $cat_posts->have_posts() ) : $cat_posts->the_post(); ?>
		...
		<article class="cat-post-item">
			...
			<header>	
				<?php
					...
					<?php
						...
						$thumbnail_size = ( empty( $instance['thumb_w'] ) || empty( $instance['thumb_h'] ) ) ? 'thumbnail' : 'cat_post_thumb_size' . $this->id;
			...
			<footer><p class="comment-num">(<?php comments_number(); ?>)</p></footer>
			<?php endif; ?>
...
?>
		...
		<p>
			<label>
				<?php _e( 'Category' ); ?>:
				<?php wp_dropdown_categories( array( 'hide_empty' => 0, 'show_option_all' => 'All Categories', 'name' => $this->get_field_name("cat"), 'selected' => $instance["cat"] ) ); ?>
			</label>
		</p>
    ...
    <p>
		...
		<p>
			<label for="<?php echo $this->get_field_id("future_posts"); ?>">
        <input type="checkbox" class="checkbox" 
          id="<?php echo $this->get_field_id("future_posts"); ?>" 
          name="<?php echo $this->get_field_name("future_posts"); ?>"
          <?php checked( (bool) $instance["future_posts"], true ); ?> />
				<?php _e( 'Show upcoming events only' ); ?>
			</label>
    </p>
